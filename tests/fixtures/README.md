...
`analysed.mp3` so the two files can be asserted against each other; built with
ffmpeg and metaflac rather than with tserato, so the tests are not reading back
